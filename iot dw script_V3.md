...
最新预生产环境添加数据表：
coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month
coss_dwd.dwd_pnw_sensor_monitored_rt_mini

coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month
coss_dwd.dwd_pnw_sensor_monitored_rt_mini

coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini
coss_ods.ods_cmsdms_pnw_permission_info_df
...
### ods_cmsdms_pnw_sensor_data_rt_mini(新增)

#### create table

```sql
drop table if exists coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini;
create table if not exists coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini (
    device_id varchar(200) not null, -- Device Id
    sensor_id varchar(200) not null, -- Sensor Id
    sensor_value numeric(20,6) null, -- Sensor Value
    time bigint not null, -- Event Time Millisecond
    ods_update_time timestamp(6) null default current_timestamp, -- Ods Update Time
    ods_load_time timestamp(6) null default current_timestamp, -- Ods Load Time
    primary key (device_id, sensor_id, event_time_ms)
) with (
    orientation=row,
    compression=no,
    storage_type=USTORE,
...
comment on table coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini is 'CMSDMS TMU Real‑time Minute Sensor Data';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.device_id is 'Device Id';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.sensor_id is 'Sensor Id';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.sensor_value is 'Sensor Value';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.time is 'Event Time Millisecond';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.event_time is 'Event Time';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.ods_update_time is 'Ods Update Time';
comment on column coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini.ods_load_time is 'Ods Load Time';
...
```python
# -*- coding: utf-8 -*-
# File : ods_cmsdms_pnw_sensor_data_rt_mini
# Author : CDM
# Date : 2026/08/28
""" CMSDMS TMU Real‑time Minute Sensor Data Acquisition and Storage Script
Function: Query unique device_id from permission table, call sensor realtime api, write data to GaussDB rt minute table
Fix: solve queue full blocking / script deadlock hang issue, add parallel consumer thread
"""
import time
import random
import requests
...
# API Basic Info
API_URL = "http://192.168.206.207:12024/api/open/share/sensor/realtime"
HEADERS = {
    "Content-Type": "application/json",
    "AppKey": "YC4kO6FS8LnFdVSilUlO3KMU",
    "request-timeout-mark": "sensor-realtime-rt-task"
}
...
DEVICE_BATCH = 1                # New api one request only support single deviceId
...
TARGET_TABLE = "coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini"
CONSTRAINT_KEYS = ["device_id", "sensor_id", "time"]  # Composite primary key
DB_PAGE_BATCH_SIZE = 1000
DB_CONSUME_BATCH = 500          # 消费线程攒多少行批量写入数据库

# Queue Control
QUEUE_MAX_SIZE = 3000           # 调小队列上限，防止内存暴涨
QUEUE_PUT_TIMEOUT = 5          # put队列超时时间，秒，超过直接失败不永久阻塞

# Thread Global Vars
api_semaphore = Semaphore(MAX_API_SEMAPHORE)
result_queue = Queue(maxsize=QUEUE_MAX_SIZE)
failed_device_list = []         # Store failed device_id for unified output after execution
# 消费线程退出哨兵
CONSUMER_SENTINEL = None
...
def fetch_single_device(device_id: str) -> None:
    """
    Single device API request: rate limiting, retry, exception classification, enqueue data
    Args:
        device_id: single device identifier string
    """
    retry_count = 0
    ...
    while retry_count <= API_RETRY_TIMES and not success_flag:
        try:
            with api_semaphore:
                ...
                if 400 <= resp.status_code < 500:
                    print_log(f"Device {device_id} 400 debug | payload={payload}, resp_body={resp.text}", level="ERROR")
                ...
                res_json = resp.json()
                # New business status code validation: code ==0 success
                if res_json.get("code") != 0:
                    raise Exception(f"Business Fail: {res_json.get('message')}")

                # Data parsing align with new response schema
                ...
                # 带超时put队列，队列满不会卡死
                result_queue.put(parse_rows, timeout=QUEUE_PUT_TIMEOUT)
                print_log(f"Device {device_id} request success, parsed sensor rows:{len(parse_rows)}")
                success_flag = True
                time.sleep(AFTER_REQ_SLEEP)

        except queue.Full:
            print_log(f"Device {device_id} queue full, put timeout, drop batch data", level="ERROR")
            failed_device_list.append(device_id)
            break
        except ValueError as client_err:
            # 4xx error, abort directly and record failed device
            err_msg = str(client_err)
            print_log(f"Device {device_id} skip, err:{err_msg}", level="ERROR")
            failed_device_list.append(device_id)
            break
        except (requests.exceptions.RequestException, ConnectionError) as net_err:
            ...
            if retry_count > API_RETRY_TIMES:
                print_log(f"Device {device_id} all retry failed, err:{err_msg}", level="ERROR")
                failed_device_list.append(device_id)
                break
            backoff_sleep = RETRY_BASE_SLEEP * (2 ** (retry_count - 1)) + random.random()
            print_log(f"Device {device_id} request fail, retry {retry_count}/{API_RETRY_TIMES}, wait {round(backoff_sleep,1)}s, err:{err_msg}", level="WARN")
            time.sleep(backoff_sleep)
        except Exception as other_err:
            # Unknown business exception, no retry
            print_log(f"Device {device_id} unknown error, drop task, err:{str(other_err)}", level="ERROR")
            failed_device_list.append(device_id)
            break


def consumer_write_db():
    """
    独立消费线程：一边读取队列，一边批量写GaussDB，解决生产消费不同步卡死
    """
    accumulate_rows = []
    while True:
        try:
            ...
            # 收到哨兵对象，退出消费线程
            if item is CONSUMER_SENTINEL:
                result_queue.task_done()
                break
            accumulate_rows.extend(item)

            # 攒够阈值批量写入
            if len(accumulate_rows) >= DB_CONSUME_BATCH:
                df = pd.DataFrame(accumulate_rows, columns=["device_id", "sensor_id", "sensor_value", "time"])
                request_json.save_to_gaussdb(
                    df,
                    config.GAUSSDB_DWS,
                    TARGET_TABLE,
                ...
                print_log(f"[Consumer thread] batch write db ok, rows:{len(accumulate_rows)}, queue_size:{result_queue.qsize()}")
                accumulate_rows.clear()
            result_queue.task_done()

        except queue.Empty:
            # 队列空，检查是否还有剩余数据，不足批次也落库
            if len(accumulate_rows) > 0:
                df = pd.DataFrame(accumulate_rows, columns=["device_id", "sensor_id", "sensor_value", "time"])
                request_json.save_to_gaussdb(
                    df,
                    config.GAUSSDB_DWS,
                ...
                print_log(f"[Consumer thread] residual data write db ok, rows:{len(accumulate_rows)}")
                accumulate_rows.clear()
            continue
        except Exception as e:
            print_log(f"[Consumer thread fatal error] {str(e)}", level="ERROR")
            time.sleep(1)
    # 退出前处理残余数据
    if len(accumulate_rows) >0:
        try:
            df = pd.DataFrame(accumulate_rows, columns=["device_id", "sensor_id", "sensor_value", "time"])
            request_json.save_to_gaussdb(df, config.GAUSSDB_DWS, TARGET_TABLE, CONSTRAINT_KEYS)
            print_log(f"[Consumer thread final flush] residual rows:{len(accumulate_rows)}")
        except Exception as e:
            print_log(f"[Consumer thread final flush fail] {str(e)}", level="ERROR")
    print_log("[Consumer thread exit]")


def get_all_device_ids(db_conn) -> list[str]:
    """Paginate to query all distinct device_id from source table, decoupled independent function"""
    count_sql = f"SELECT COUNT(DISTINCT device_id) FROM {SOURCE_TABLE}"
    total_device = db_conn.fetch_data(count_sql)[0][0]
    print_log(f"Total distinct device_id in source table: {total_device}")

    all_devices = []
    offset = 0
    while offset < total_device:
        page_sql = f"""
        ...
        page_devices = [row[0] for row in page_res if row[0]]
        all_devices.extend(page_devices)
    print_log(f"All device_id loaded, valid count: {len(all_devices)}")
    return all_devices


def batch_submit_task(executor: ThreadPoolExecutor, device_list: list[str]) -> None:
    """Submit thread tasks in batches to smooth traffic, avoid creating massive futures at once"""
    task_futures = []
    for dev_id in device_list:
        task_futures.append(executor.submit(fetch_single_device, dev_id))
...
# ===================== Main Entry =====================
def fetch_and_save_realtime_sensor_data():
    ...
    try:
        device_id_list = get_all_device_ids(db_conn)
        print_log(f"Total device to request api: {len(device_id_list)}")
        init_cost = round(time.time() - init_start, 2)

        # 启动独立数据库消费线程
        consumer_thread = Thread(target=consumer_write_db, daemon=True)
        consumer_thread.start()
        print_log("Consumer write db thread started")
        ...
        with ThreadPoolExecutor(max_workers=MAX_API_SEMAPHORE) as pool_executor:
            batch_submit_task(pool_executor, device_id_list)
        api_cost = round(time.time() - api_start, 2)
        print_log("All api worker threads finished, send sentinel to stop consumer")

        # 生产者全部结束，放入哨兵通知消费线程退出
        result_queue.put(CONSUMER_SENTINEL)
        # 等待队列全部消费完毕
        result_queue.join()
        # 等待消费线程安全退出
        consumer_thread.join(timeout=30)

        # Print failed device summary
        if failed_device_list:
            print_log(f"Task finished, total failed device count: {len(failed_device_list)}", level="WARN")
            print_log(f"Failed device sample (first 5): {failed_device_list[:5]}", level="WARN")
        else:
            print_log("All device api request success, no failed task")

        data_proc_cost = round(time.time() - api_start - db_write_cost, 2)
...
if __name__ == "__main__":
    fetch_and_save_realtime_sensor_data()
```
...
### ods_cmsdms_pnw_permission_info_df(新增)

#### create table 

```sql
drop table if exists coss_ods.ods_cmsdms_pnw_permission_info_df;
create table if not exists coss_ods.ods_cmsdms_pnw_permission_info_df (
    device_id varchar(200) not null, -- Device Id
    device_code varchar(200) null, -- Device Code
    device_name varchar(200) null, -- Device Name
    org_id varchar(200) not null, -- Org Id
    business_type varchar(120) null, -- Business Type
    group_id varchar(200) not null, -- Group Id
    group_name varchar(200) null, -- Group Name
    longitude numeric(18,8) null, -- Longitude
    latitude numeric(18,8) null, -- Latitude
    coordinate_system varchar(64) null, -- Coordinate System
    tags_json text null, -- Tags Json
    sensor_id varchar(200) not null, -- Sensor Id
    sensor_code varchar(200) null, -- Sensor Code
    sensor_name varchar(200) null, -- Sensor Name
    "unit" varchar(120) null, -- Unit
    sensor_value_type varchar(64) null, -- Sensor Value Type
    sensor_tag varchar(200) null, -- Sensor Tag
    ods_update_time timestamp(6) null default current_timestamp, -- Ods Update Time
    ods_load_time timestamp(6) null default current_timestamp, -- Ods Load Time
    primary key (device_id, org_id, group_id, sensor_id)
) with (
...
comment on table coss_ods.ods_cmsdms_pnw_permission_info_df is 'CMSDMS Permission Information';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.device_id is 'Device Id';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.device_code is 'Device Code';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.device_name is 'Device Name';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.org_id is 'Org Id';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.business_type is 'Business Type';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.group_id is 'Group Id';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.group_name is 'Group Name';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.longitude is 'Longitude';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.latitude is 'Latitude';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.coordinate_system is 'Coordinate System';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.tags_json is 'Tags Json';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.sensor_id is 'Sensor Id';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.sensor_code is 'Sensor Code';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.sensor_name is 'Sensor Name';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df."unit" is 'Unit';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.sensor_value_type is 'Sensor Value Type';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.sensor_tag is 'Sensor Tag';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.ods_update_time is 'Ods Update Time';
comment on column coss_ods.ods_cmsdms_pnw_permission_info_df.ods_load_time is 'Ods Load Time';
```
...
```python
# -*- coding: utf-8 -*-
# File : ods_cmsdms_pnw_permission_info_df
# Date : 2026/08/26
""" CMSDMS Device Permission Sensor Data Synchronization Script """
import time
import random
...
# ======================== Global Configuration Area (Centralized management of all hardcoded values) ========================
# API base address and request headers
BASE_URL = "http://192.168.206.207:12024"
API_PATH = "/api/open/share/permission"
FULL_API_URL = f"{BASE_URL}{API_PATH}"
HEADERS = {
    "Content-Type": "application/json",
    "AppKey": "YC4kO6FS8LnFdVSilUlO3KMU",
...
QUEUE_MAX_CAP = 500
# Target database table and composite primary key
TARGET_TABLE = "coss_ods.ods_cmsdms_pnw_permission_info_df"
CONSTRAINT_KEYS = ["device_id", "org_id", "group_id", "sensor_id"]
# Global thread‑safe containers
data_queue = Queue(maxsize=QUEUE_MAX_CAP)
fail_page_list = []
...
def get_total_page_count() -> int:
    ...
    while retry_count <= MAX_RETRY_TIMES:
        try:
            ...
            print_log(f"init request status_code:{resp.status_code}, response_text:{resp.text[:1000]}")

            if 400 <= resp.status_code < 500:
                raise ValueError(f"Init request 4xx error, status={resp.status_code}, resp={resp.text}")
            if resp.status_code >= 500:
                raise ConnectionError(f"Init request 5xx error, status={resp.status_code}")
            ...
            if retry_count > MAX_RETRY_TIMES:
                raise net_err
            sleep_sec = RETRY_BACKOFF_BASE * (2 ** (retry_count - 1)) + random.random()
            print_log(f"Init request retry {retry_count}, sleep {round(sleep_sec,1)}s, err:{str(net_err)}", level="WARN")
            time.sleep(sleep_sec)

    raise Exception("get_total_page_count exceed max retry times")
    ...
    while retry_count <= MAX_RETRY_TIMES and not task_success:
        try:
            with req_semaphore:
                ...
                if 400 <= resp.status_code < 500:
                    raise ValueError(f"4xx client error, status code {resp.status_code}, resp_text:{resp.text}, no retry required")
                if resp.status_code >= 500:
                    raise ConnectionError(f"5xx server error, status code {resp.status_code}")
                ...
                for device in records:
                    ...
                    dev_code = device.get("deviceCode", "")
                    dev_name = device.get("deviceName", "")
                    org_id = device.get("orgCode", "")
                    bus_type = device.get("businessType", "")
                    group_id = str(device.get("groupId", ""))
                    group_name = device.get("groupName", "")
                    longitude = device.get("longitude")
                    latitude = device.get("latitude")
                    coordinate_system = device.get("coordinateSystem", "")
                    tags_obj = device.get("tags", {})
                    tags_json_str = json.dumps(tags_obj, ensure_ascii=False)
                    sensor_arr = device.get("sensors", [])
                    for sensor in sensor_arr:
                        row = [
                            dev_id,
                            ...
                            sensor.get("sensorCode", ""),
                            sensor.get("sensorName", ""),
                            sensor.get("unit", "").strip() if sensor.get("unit") else "",
                            sensor.get("valueType", ""),
                            sensor.get("tag", "")
                        ]
                        page_rows.append(row)
                data_queue.put(page_rows)
...
def batch_submit_thread_tasks(executor: ThreadPoolExecutor, page_list: list[int]) -> None:
    ...
    total_page = len(page_list)
    if total_page <= 0:
        print_log("No pages need submit")
        return
    for start_idx in range(0, total_page, BATCH_SUBMIT_SIZE):
        batch_pages = page_list[start_idx: start_idx + BATCH_SUBMIT_SIZE]
        print_log(f"Submit page batch: {batch_pages[0]} ~ {batch_pages[-1]}, batch size {len(batch_pages)}")
...
def sync_permission_sensor_data():
    ...
    try:
        ...
        print_log(f"Single request throttling interval: {REQUEST_INTERVAL}s, max retry times: {MAX_RETRY_TIMES}")
        print_log(f"Target database table: {TARGET_TABLE}")
        print_log(f"Api endpoint: {FULL_API_URL}")
        print("=======================================================================")
        api_start_ts = time.time()
        all_page_nums = list(range(1, total_pages + 1))
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
...
### dwd_pnw_sensor_monitored_hst_mini_month(新增)

#### create  table

```sql
drop table if exists coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month;
create table if not exists coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month (
...
comment on table coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month is 'Sensor Monitoring History Data';
comment on column coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month.device_id is 'Device ID';
comment on column coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month.sensor_id is 'Sensor ID';
comment on column coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month.sensor_value is 'Sensor Value';
comment on column coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month.sensor_time is 'Sensor Time';
comment on column coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month.dwd_update_time is 'Data Update Time';
comment on column coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month.dwd_load_time is 'Data Loading Time';
...
```sql
-- ****************************************************************************************
-- Subject     Areas: Pipe Network
-- Function Describe: Pipe Network For Sensor Monitoring History Data
-- Create       Date: 2026-08-31
-- Modify Date                Modify By                    Modify Content
-- None                       None                         None
-- Source Table:  coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini
-- Target Table:  coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month
-- ****************************************************************************************
insert into coss_dwd.dwd_pnw_sensor_monitored_hst_mini_month
select
    device_id,
    sensor_id,
    sensor_value,
    to_timestamp(time::bigint / 1000.0) as sensor_time,
    current_timestamp as dwd_update_time,
    current_timestamp as dwd_load_time
from coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini
...
### dwd_pnw_sensor_monitored_rt_mini(新增)

#### create table

```sql
drop table if exists coss_dwd.dwd_pnw_sensor_monitored_rt_mini;
create table if not exists coss_dwd.dwd_pnw_sensor_monitored_rt_mini (
    device_id       varchar(100),
    sensor_id       varchar(100),
    sensor_value    decimal(20,6),
    sensor_time     timestamp(6),
...
comment on table coss_dwd.dwd_pnw_sensor_monitored_rt_mini is 'Sensor Monitoring Real Time Data';
comment on column coss_dwd.dwd_pnw_sensor_monitored_rt_mini.device_id is 'Device ID';
comment on column coss_dwd.dwd_pnw_sensor_monitored_rt_mini.sensor_id is 'Sensor ID';
comment on column coss_dwd.dwd_pnw_sensor_monitored_rt_mini.sensor_value is 'Sensor Value';
comment on column coss_dwd.dwd_pnw_sensor_monitored_rt_mini.sensor_time is 'Sensor Time';
comment on column coss_dwd.dwd_pnw_sensor_monitored_rt_mini.dwd_update_time is 'Data Update Time';
comment on column coss_dwd.dwd_pnw_sensor_monitored_rt_mini.dwd_load_time is 'Data Loading Time';
...
-- ****************************************************************************************
-- Subject     Areas: Pipe Network
-- Function Describe: Pipe Network For Sensor Monitoring Real Time Data
-- Create       Date: 2026-08-31
-- Modify Date                Modify By                    Modify Content
-- None                       None                         None
-- Source Table:  coss_ods.ods_cmsdms_pnw_sensor_data_rt_mini
-- Target Table:  coss_dwd.dwd_pnw_sensor_monitored_rt_mini
-- ****************************************************************************************
insert into coss_dwd.dwd_pnw_sensor_monitored_rt_mini
select
    device_id,
    sensor_id,
    sensor_value,
...
## dim_cmsdms_device_related_dict
...
### coss_dm.dm_rws_catchwater_image_hst_mini_month（待开发）

```sql
drop table if exists coss_dm.dm_rws_catchwater_image_hst_mini_month;

create table if not exists coss_dm.dm_rws_catchwater_image_hst_mini_month (
...
comment on table coss_dm.dm_rws_catchwater_image_hst_mini_month is 'Catchwater Monitoring Image History Data';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.id              is 'ID';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.device_id       is 'Device ID';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.device_code     is 'Device Code';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.utc_time        is 'UTC Time';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.photo           is 'Photo Url';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.dm_update_time   is 'Update Time';
comment on column coss_dm.dm_rws_catchwater_image_hst_mini_month.dm_load_time     is 'Load Time';

```

### coss_dm.dm_rws_catchwater_image_rt_mini（待开发）

```sql
drop table if exists coss_dm.dm_rws_catchwater_image_rt_mini;
create table if not exists coss_dm.dm_rws_catchwater_image_rt_mini (
    id              varchar(50) not null,
    device_id       varchar(50) not null,
    device_code     varchar(120),
    utc_time        timestamp(6),
...
comment on table coss_dm.dm_rws_catchwater_image_rt_mini is 'Catchwater Monitoring Image Real Time Data';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.id              is 'ID';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.device_id       is 'Device ID';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.device_code     is 'Device Code';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.utc_time        is 'UTC Time';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.photo           is 'Photo Url';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.dm_update_time   is 'Update Time';
comment on column coss_dm.dm_rws_catchwater_image_rt_mini.dm_load_time     is 'Load Time';

```



### dm_rws_sensor_monitored_hst_mini_month（新增）
...
-- ****************************************************************************************
-- Subject     Areas: Pipe Network
-- Function Describe: Pipe Network For Sensor Monitoring History Data(Water Level)
-- Create       Date: 2026-08-31
-- Modify Date                Modify By                    Modify Content
-- None                       None                         None
-- Source Table:  coss_dwd.dwd_pnw_sensor_monitored_rt_mini
-- Source Table:  coss_dim.dim_device_monitoring_point_info
-- Target Table:  coss_dm.dm_rws_sensor_monitored_hst_mini_month
-- ****************************************************************************************
insert into coss_dm.dm_rws_sensor_monitored_hst_mini_month 
select
    t.device_id,
...
from coss_dwd.dwd_pnw_sensor_monitored_rt_mini t
inner join coss_dim.dim_device_monitoring_point_info t1 on t.device_id = t1.device_id and t1.business_code = 'DE_BU_000001'
where t.sensor_id = 'level' and dwd_update_time >= '${dm_update_time}'
on duplicate key update nothing
```

### dm_rws_sensor_monitored_rt_mini（新增）
...
-- ****************************************************************************************
-- Subject     Areas: Pipe Network
-- Function Describe: Pipe Network For Sensor Monitoring Real Time Data(Water Level)
-- Create       Date: 2026-08-31
-- Modify Date                Modify By                    Modify Content
-- None                       None                         None
-- Source Table:  coss_dwd.dwd_pnw_sensor_monitored_rt_mini
-- Source Table:  coss_dim.dim_device_monitoring_point_info
-- Target Table:  coss_dm.dm_rws_sensor_monitored_rt_mini
-- ****************************************************************************************
