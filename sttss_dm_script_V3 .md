...
left join coss_dwd.dwd_ass_channels_df t1 on t.option_no = t1.option_no and t.ch_id = t1.ch_id
...
left join coss_dwd.dwd_ass_rw_src_df t1 on t.rw_id = t1.rw_id
where
t1.rw_id != 'RW00000001'
;
...
select
    ...
    if((t.current_storage + t1.current_storage) = 0 or (t.current_storage + t1.current_storage) is null, 0, (t.change_storage + t1.change_storage) / (t.current_storage + t1.current_storage) * 100) as total_change_storage,
...
;
-- 删除不完全的数据
-- delete from coss_dm.dm_rws_daily_rw_yield_di where statistical_day >= (select max(statistical_day) - 100 as statistical_day from coss_dm.dm_rws_daily_rw_yield_di);
...
select
    ...
    case
        when t1.p_qty is null or t1.p_qty = 0 then 0.0  
...
select
    ...
    dt
from coss_dws.dws_rws_rw_supply_detail_di_year
  where rec_dt >= date_trunc('month', to_date('${rec_dt}','YYYY-MM-DD HH24:MI:SS.FF3')) - interval '1 year'
...
select
    ...
    '%' as unit,
    current_timestamp as etl_time,  -- 替换localtimestamp为current_timestamp
    t.mh as mh,
    current_timestamp dm_update_time,
    current_timestamp dm_load_time
...
-- Precomputation Metrics of propose and quantity delivery volume
drop table if exists coss_tmp.dm_rws_region_year_kpi_dip_01;
create table if not exists coss_tmp.dm_rws_region_year_kpi_dip_01 as
select
    region_code as region,
    sum(p_qty) as p_qty,
    sum(qty_del) as qty_del,
...
drop table if exists coss_tmp.dm_rws_region_year_kpi_dip_02;
create table if not exists coss_tmp.dm_rws_region_year_kpi_dip_02 as
select
    'HKSAR' as region,
    sum(p_qty) as p_qty,
    sum(qty_del) as qty_del,
...
drop table if exists coss_tmp.dm_rws_region_year_kpi_dip_03;
create table if not exists coss_tmp.dm_rws_region_year_kpi_dip_03 as
select
    region as region,
...
drop table if exists coss_tmp.dm_rws_region_year_kpi_dip_04;
create table if not exists coss_tmp.dm_rws_region_year_kpi_dip_04 as
select
    region as region,
    round(dt / 10000) as yr,
    sum(p_qty) / count(1) as p_qty,
...
select
    uuid() as id,
    region as region,
    'bi_p_250' as item_code,
    'Raw Water Propose Supply Volume' as item_name,
    sum(p_qty) as item_value,
    'Ml' as unit,
    current_timestamp as etl_time,  
    round(t.dt / 10000) as yr,
    current_timestamp dm_update_time,
    current_timestamp dm_load_time
from coss_tmp.dm_rws_region_year_kpi_dip_02 t
group by
    region,
    round(t.dt / 10000)  
...
select
    ...
    current_timestamp dm_update_time,
    current_timestamp dm_load_time
from coss_tmp.dm_rws_region_year_kpi_dip_01 t
group by
    region,
    round(t.dt / 10000)
...
select
    uuid() as id,
    region as region,
    'bi_p_251' as item_code,
    'Raw Water Actual Supply Volume' as item_name,
    sum(qty_del) as item_value,
    'Ml' as unit,
    current_timestamp as etl_time, 
    round(t.dt / 10000) as yr,
    current_timestamp dm_update_time,
...
select
    ...
    current_timestamp dm_update_time,
    current_timestamp dm_load_time
from coss_tmp.dm_rws_region_year_kpi_dip_04 t
left join coss_tmp.dm_rws_region_year_kpi_dip_04 t1 on t.yr = t1.yr + 1
...
select
    ...
    current_timestamp dm_update_time,
    current_timestamp dm_load_time
from coss_tmp.dm_rws_region_year_kpi_dip_03 t
left join coss_tmp.dm_rws_region_year_kpi_dip_03 t1 on t.region = t1.region and t.yr = t1.yr + 1
...
## 3.coss_dm.dm_srs_annual_sr_pool_stopped_di
...
-- 修正临时表名：原coss_tmp.dm_rws_annual_sr_pool_stopped_di_01改为与创建时一致的coss_tmp.dm_srs_annual_sr_pool_stopped_di_01
insert into coss_tmp.dm_srs_annual_sr_pool_stopped_di_01
select
